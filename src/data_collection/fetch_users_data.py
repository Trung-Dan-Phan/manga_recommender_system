...
def fetch_user_data(username: str) -> tuple:
    ...
    if response.status_code != 200:
        logger.error(f"Error fetching data for {username}: {response.status_code}, {response.text}")
        return {}, []
    ...
    user_statistics = {
        ...
        "Favorite Genres": ", ".join(f"{g['genre']} ({g['count']})" for g in manga_stats.get("genres", [])) if "genres" in manga_stats else None,
        "Most Read Years": ", ".join(f"{y['chaptersRead']} chapters ({y['count']} manga, {y['meanScore']} avg score)" for y in manga_stats.get("releaseYears", [])) if "releaseYears" in manga_stats else None,
        "Tag Preferences": ", ".join(f"{t['chaptersRead']} chapters ({t['count']} manga, {t['meanScore']} avg score)" for t in manga_stats.get("tags", [])) if "tags" in manga_stats else None,
        "Staff Preferences": ", ".join(f"{s['chaptersRead']} chapters ({s['count']} manga, {s['meanScore']} avg score)" for s in manga_stats.get("staff", [])) if "staff" in manga_stats else None,
        "Preferred Manga Lengths": ", ".join(f"{l['chaptersRead']} chapters ({l['count']} manga, {l['meanScore']} avg score)" for l in manga_stats.get("lengths", [])) if "lengths" in manga_stats else None
    }
    ...
    for list_data in data["data"]["MediaListCollection"]["lists"]:
        for entry in list_data["entries"]:
            user_manga_list.append({
                ...
                "Genres": ", ".join(entry["media"]["genres"]) if entry["media"]["genres"] else None,
                "Format": entry["media"]["format"],
                "Score": entry["score"],
                "Progress": entry["progress"],
                "Status": entry["status"],
                "Priority": entry["priority"],
                "Custom Lists": ", ".join(entry["customLists"]) if entry["customLists"] else None,
                "Started Reading": entry["startedAt"]["year"] if entry["startedAt"] else None,
                "Completed Reading": entry["completedAt"]["year"] if entry["completedAt"] else None,
    ...
    if user_manga_list:
        logger.info(f"Fetched manga list for {username} - First Entry: {user_manga_list[0]}")
    else:
...
if __name__ == "__main__":
  ...
  users_df = pd.read_csv('./data/raw/anilist_users_v3.csv')
  ...
  for index, user in enumerate(usernames, start=1):
      ...
      logger.info(f"Fetching data for {user} ({index}/{total_users}) - {remaining_users} users remaining...")
  ...
  # Save to CSVs
  df_stats.to_csv("./data/raw/user_statistics_v3.csv", index=False)
  df_manga.to_csv("./data/raw/user_manga_list_v3.csv", index=False)
