...
def concatenate_csv_batches(
    batch_folder: str, 
    batch_start_name: str, 
    output_folder: str, 
    output_filename: str
    ):
    """
    Concatenates multiple CSV batch files into a single CSV file and saves it in a specified directory.
    ...
    csv_files = sorted([f for f in os.listdir(batch_folder) if f.startswith(batch_start_name) and f.endswith(".csv")])
    ...
    logger.info(f"Concatenation complete! Final file saved at {output_path} with {len(final_df)} rows.")


if __name__ == "__main__":
    batch_start_name = 'user_manga_list'
    concatenate_csv_batches(batch_folder=BATCH_FOLDER, batch_start_name=batch_start_name, output_folder=OUTPUT_FOLDER, output_filename=OUTPUT_FILENAME)
